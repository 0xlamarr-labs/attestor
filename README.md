...
| `ATTESTOR_CONTROL_PLANE_PG_URL` | Optional PostgreSQL connection URL for the shared hosted control-plane first slice (accounts, account users, account sessions, account user action tokens, tenant keys, usage, billing entitlements, admin audit, admin idempotency replay, Stripe webhook dedupe) |
...
| Expanded verification surface | 1394 tests across 10 suites: 557 unit + 492 live API + 43 live PostgreSQL + 38 connector/filing + 98 healthcare E2E + 35 control-plane backup/restore + 42 control-plane backup/restore shared PG + 83 live shared control-plane PG + 3 live Cypress connectivity + 3 live VSAC connectivity, plus env-gated live Snowflake and full ONC/VSAC credential runs |
