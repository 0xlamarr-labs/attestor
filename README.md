...
| Next control-plane track | The active frozen track is [docs/02-architecture/release-policy-control-plane-buildout.md](docs/02-architecture/release-policy-control-plane-buildout.md), focused on policy packs, signed bundle distribution, scoped activation, simulation, rollback, and tenant-aware rollout on top of the packaged release-layer surface; Steps `01-07 / 20` are complete in `src/release-policy-control-plane/types.ts`, `src/release-policy-control-plane/object-model.ts`, `src/release-policy-control-plane/scoping.ts`, `src/release-policy-control-plane/bundle-format.ts`, `src/release-policy-control-plane/bundle-signing.ts`, `src/release-policy-control-plane/store.ts`, and `src/release-policy-control-plane/activation-records.ts`. |
