...
- Full enterprise IAM/session management is not shipped (TOTP MFA + SMTP invite/reset delivery first slices are now present, but WebAuthn/passkeys and `SSO/SAML` are still absent).
...
| Expanded verification surface | 1580 tests across 17 suites: 557 unit + 541 live API + 30 live account email delivery + 43 live PostgreSQL + 38 connector/filing + 98 healthcare E2E + 38 control-plane backup/restore + 51 control-plane backup/restore shared PG + 114 live shared control-plane PG + 21 live OTLP export + 8 observability bundle + 12 live shared Redis rate-limit + 11 live async tenant execution Redis + 12 live multi-node HA proxy + 3 live Cypress connectivity + 3 live VSAC connectivity, plus env-gated live Snowflake and full ONC/VSAC credential runs |
