...
| Next control-plane track | The active frozen track is [docs/02-architecture/release-policy-control-plane-buildout.md](docs/02-architecture/release-policy-control-plane-buildout.md), focused on policy packs, signed bundle distribution, scoped activation, simulation, rollback, and tenant-aware rollout on top of the packaged release-layer surface; Step `01 / 20` is complete in `src/release-policy-control-plane/types.ts`. |
