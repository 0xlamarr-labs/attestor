...
| Next control-plane track | The frozen [release-policy control-plane buildout](docs/02-architecture/release-policy-control-plane-buildout.md) is now `20 / 20` complete. The control plane is packaged behind `attestor/release-policy-control-plane`, the service layer now consumes that curated surface instead of deep internal imports, and future work should be justified by either a second proving domain or a real external/operator-managed deployment boundary rather than by more core primitive churn. |
