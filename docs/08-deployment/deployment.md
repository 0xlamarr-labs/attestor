...
| `ATTESTOR_CONTROL_PLANE_PG_URL` | No | None | Explicit shared PostgreSQL control-plane first slice for hosted accounts, account users, account sessions, tenant keys, usage, billing entitlements, admin audit, admin idempotency replay, and Stripe webhook dedupe |
