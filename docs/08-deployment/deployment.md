...
- Observability first slice: W3C trace-context-compatible response headers, Prometheus-text metrics at `GET /api/v1/admin/metrics` plus `GET /api/v1/metrics`, `GET /api/v1/admin/telemetry` exporter status plus email-delivery status, optional JSONL request logs via `ATTESTOR_OBSERVABILITY_LOG_PATH`, optional OTLP logs + trace + metrics export over HTTP/protobuf, and a bundled persisted Collector + Prometheus + Alertmanager + Grafana + Tempo + Loki stack via `docker-compose.observability.yml`, `ops/otel/collector-config.yaml`, and `ops/observability/`
- HA runtime truth: all API responses include `x-attestor-instance-id`, `GET /api/v1/health` and `GET /api/v1/ready` expose `instanceId` and `highAvailability` status for load-balancer debugging and readiness gating, and workers now expose `/health` + `/ready` on `ATTESTOR_WORKER_HEALTH_PORT`
...
- Final environment-specific autoscaling/SLO tuning beyond the shipped Kubernetes HA bundle and provider overlays
