...
The repository already proves the first two lines. Step 07 proves that one API runtime can cut the request path over to the shared authority-store contract when the PostgreSQL substrate is configured and reachable. Step 08 now proves the third line under multi-instance concurrency, restart, reconnect, and recovery pressure using the embedded PostgreSQL harness. Step 09 remains to align promotion docs, readiness packets, and anti-overclaim gates.
...
| In progress | 0 |
| Not started | 1 |
| Current posture | Step 08 is complete: the API bootstrap can cut `production-shared` over to the async shared authority-store request path when `ATTESTOR_RELEASE_AUTHORITY_PG_URL` points at a reachable PostgreSQL substrate, and `tests/production-shared-multi-instance-recovery.test.ts` now proves two API runtimes sharing the same PostgreSQL authority plane under concurrent policy mutations, reviewer-queue claims, token use/revocation, and pool close/reconnect recovery. Runtime bootstrap still probes all eight shared authority stores, seeds the finance proving policy into the shared policy store, marks component metadata `bootstrapWired=true` only after the request path is actually wired, and reports `requestPathUsesSharedStores=true` only when all runtime store modes are `shared` and the explicit `async-shared-authority-stores` contract is active. Missing shared store configuration, unreachable PostgreSQL, contract mismatch, or remaining guard blockers still keep non-preflight API paths fail-closed. The remaining work is Step 09 promotion/readiness documentation and anti-overclaim gates. |
...
Step 09 is next: align production promotion docs, readiness packets, and anti-overclaim gates with the now-tested shared authority plane without claiming more than the embedded PostgreSQL multi-instance/reconnect proof establishes.
