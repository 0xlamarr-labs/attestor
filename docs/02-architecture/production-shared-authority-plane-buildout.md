...
The repository already proves the first two lines. Step 07 now proves that one API runtime can cut the request path over to the shared authority-store contract when the PostgreSQL substrate is configured and reachable. Step 08 still has to prove the third line under multi-instance concurrency, restart, reconnect, and recovery pressure.
...
Step 08 is next: prove the shared authority plane with multiple API runtimes, concurrent claims/mutations, restart, reconnect, token use/revocation, and policy mutation recovery before Step 09 promotes the production readiness packets.
