...
| In progress | 1 |
| Not started | 2 |
| Current posture | Step 07 is in progress: the API health/readiness surface now has a shared-authority runtime readiness diagnostic that initializes and probes all PostgreSQL-backed shared authority stores without overclaiming production-shared readiness. The request-path cutover truth now comes from release-runtime bootstrap diagnostics and still reports `requestPathUsesSharedStores=false`; shared store component metadata still reports `bootstrapWired=false`, and production-shared HTTP request handling is therefore guarded fail-closed for non-preflight API paths until every ready shared store is explicitly marked as runtime bootstrap wired and the request path explicitly reports shared-store contract cutover. |
