...
| In progress | 0 |
| Not started | 4 |
| Current posture | Step 05 is complete: release token introspection and release evidence packs now have PostgreSQL-backed shared stores on the release-authority substrate. Token lifecycle records preserve issued, consumed, revoked, expired, and replay-use state under row-locked mutation, while evidence packs verify DSSE signatures and bundle digests on read and write. Runtime bootstrap wiring still remains for the later production-shared cutover steps. |
...
Step 06 is next: promote degraded-mode grants, policy bundle state, activation approvals, and policy mutation audit history onto shared authoritative storage so release and policy truth no longer diverge across nodes.
