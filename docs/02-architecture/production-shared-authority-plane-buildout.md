...
| In progress | 0 |
| Not started | 5 |
| Current posture | Step 04 is complete: the shared release reviewer queue now has a PostgreSQL-backed implementation on the release-authority substrate, with deterministic pending reads and explicit multi-consumer claim discipline based on short transactional `FOR UPDATE SKIP LOCKED` selection. The runtime still truthfully stops at `single-node-durable` until the shared stores are actually wired through bootstrap in the later runtime steps. |
...
Step 05 is next: promote release token introspection and release evidence packs onto the shared release-authority PostgreSQL substrate so verification, replay checks, token lifecycle, and proof-carrying records share the same authority truth across runtimes.
