...
| 07 | in_progress | Wire `production-shared` bootstrap, health, and readiness truth | `src/service/bootstrap/release-runtime.ts`, `src/service/bootstrap/shared-authority-readiness.ts`, `src/service/http/routes/core-routes.ts`, `src/service/bootstrap/api-route-runtime.ts`, `tests/shared-authority-runtime-readiness.test.ts`, `tests/production-runtime-profile.test.ts`, `package.json`, `docs/02-architecture/production-shared-authority-plane-buildout.md` | Shared-authority readiness now instantiates and probes all eight PostgreSQL-backed shared stores and exposes the result through health/readiness diagnostics. The request-path cutover signal is owned by release-runtime bootstrap diagnostics instead of a hidden static readiness constant. It intentionally keeps `ready=false` while the shared store component metadata still has `bootstrapWired=false` and while the runtime request path reports `requestPathUsesSharedStores=false`, so the runtime cannot confuse "shared stores exist" with "production-shared request-path cutover is complete." The remaining Step 07 work is the real runtime bootstrap cutover, not a status-label flip. |
