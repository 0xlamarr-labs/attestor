...
| In progress | 1 |
| Not started | 2 |
| Current posture | Step 07 is in progress: the API can now boot in `production-shared` preflight mode without claiming readiness. The health/readiness surface exposes the runtime durability violations and shared-authority runtime diagnostic. The request-path cutover truth comes from release-runtime bootstrap diagnostics and still reports `requestPathUsesSharedStores=false`; the diagnostic now requires both all-shared runtime store modes and an explicit `async-shared-authority-stores` contract before it can report true. Shared store component metadata still reports `bootstrapWired=false`, and production-shared HTTP request handling is guarded fail-closed for non-preflight API paths until every ready shared store is explicitly marked as runtime bootstrap wired and the request path explicitly reports shared-store contract cutover. |
...
| 07 | in_progress | Wire `production-shared` bootstrap, health, and readiness truth | `src/service/bootstrap/release-runtime.ts`, `src/service/bootstrap/shared-authority-readiness.ts`, `src/service/bootstrap/production-shared-request-guard.ts`, `src/service/http/routes/core-routes.ts`, `src/service/bootstrap/api-route-runtime.ts`, `src/service/bootstrap/routes.ts`, `tests/shared-authority-runtime-readiness.test.ts`, `tests/production-shared-request-guard.test.ts`, `tests/production-shared-preflight-bootstrap.test.ts`, `tests/production-runtime-profile.test.ts`, `package.json`, `docs/02-architecture/production-shared-authority-plane-buildout.md` | Shared-authority readiness now instantiates and probes all eight PostgreSQL-backed shared stores and exposes the result through health/readiness diagnostics. The API runtime can boot in `production-shared` preflight mode, but readiness stays false while release runtime durability still has shared-store violations. The request-path cutover signal is owned by release-runtime bootstrap diagnostics instead of a hidden static readiness constant, and it now requires both all-shared runtime store modes and the explicit `async-shared-authority-stores` contract before `requestPathUsesSharedStores` can become true. A production-shared HTTP request guard keeps non-preflight API paths fail-closed while shared store component metadata reports `bootstrapWired=false` and the request path reports `requestPathUsesSharedStores=false`, so the runtime cannot confuse "shared stores exist" with "production-shared request-path cutover is complete." The remaining Step 07 work is the real async shared-store runtime bootstrap cutover, not a status-label flip. |
...
Step 07 is in progress. The next sub-step is the real runtime bootstrap cutover: adapt the release/policy authority contracts to the explicit `async-shared-authority-stores` boundary, then mark shared store component metadata as runtime bootstrap wired only after the API request path actually consumes those shared contracts.
