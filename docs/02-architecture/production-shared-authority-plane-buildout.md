...
| In progress | 0 |
| Not started | 3 |
| Current posture | Step 06 is complete: degraded-mode grants, policy bundle state, policy activation approvals, and policy mutation audit history now have PostgreSQL-backed shared stores on the release-authority substrate. Degraded-mode mutations serialize through advisory locking plus grant row locks, and policy mutation audit appends preserve contiguous hash-linked sequence under concurrency. Runtime bootstrap wiring still remains for the later production-shared cutover steps. |
...
Step 07 is next: wire the `production-shared` bootstrap so the runtime actually instantiates the shared stores, exposes readiness diagnostics honestly, and continues to fail closed when any required shared authority component is missing or degraded.
