...
| In progress | 0 |
| Not started | 1 |
| Current posture | Step 07 is complete: the `single-node-durable` profile now has an integrated restart/recovery guard proving release decisions, reviewer queue state, token introspection, evidence packs, policy control-plane state, policy activation approvals, policy mutation audit logs, and degraded-mode grants all reload together after runtime bootstrap restart. The default `local-dev` profile still keeps fast in-memory release authority stores; the `single-node-durable` profile satisfies its release runtime durability requirements. |
...
Step 08 should update production docs and readiness gates with the runtime profile matrix, operator knobs, restart/recovery command path, remaining single-node limits, and anti-overclaim language before this track is closed.
